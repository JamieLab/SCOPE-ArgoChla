...
argo_file = 'D:/Data/ARGO/argo_bio-profile_index.txt' #File containing all the current bio-argo profiles (this needs to be downloaded periodically)
...
argo_loc = 'D:/Data/ARGO/data' # Local location to save files.
...
for k in range(len(lat_bounds)):
    ...
    for i in range(0,len(argo_list)):
        ...
        if (checkfileexist(os.path.join(argo_loc,argo_list['file'].iloc[i])) == True) & (argo_list['file'].iloc[i].split('/')[-1][1] != 'R'):
            ...
            if 'CHLA_ADJUSTED' in c.variables.keys():
                ...
                # data_mode = np.array(c['DATA_MODE'])
                #
                # o = []
    ...
    #print(chl)
    chl = np.array(chl)
    np.savetxt('csv/'+output_files[k]+".csv", chl, delimiter=",")
